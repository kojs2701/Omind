...
async function callGeminiAPI(prompt, imageDataUrl = null) {
    const maxRetries = 3;
    let lastError;
...
function createLocalRelatedProblem(topic, subject) {
    return {
        title: `${subject} 관련 유형 연습문제`,
        problemText: `${topic || '최근 오답 주제'}와(과) 유사한 개념을 점검하는 문제입니다. 가장 적절한 설명을 고르세요.`,
        options: [
            '핵심 개념을 먼저 정리하고 풀이 순서를 세운다.',
            '정답만 외우고 개념은 생략한다.',
            '문제를 읽지 않고 계산부터 시작한다.',
            '오답 원인을 확인하지 않고 새 문제만 푼다.',
            '검산 없이 바로 제출한다.'
        ],
        correctAnswerIndex: 0,
        explanation: '관련 유형 문제는 오답 원인-개념-적용 순서로 접근하면 정답률이 높아집니다.',
        subject: subject || '수학',
        difficulty: '중',
        solved: false,
        createdAt: firebase.firestore.FieldValue.serverTimestamp()
...
function buildLocalLearningPlanText() {
    const avg = scores.length > 0
        ? (scores.reduce((a, b) => a + Number(b.percentage || 0), 0) / scores.length).toFixed(1)
        : '미정';
    return [
        `약점 부분: 최근 오답 주제를 중심으로 핵심 개념 3개를 재정리하세요. (현재 평균 ${avg}%)`,
        '복습 일정: 오늘, 1일 후, 3일 후, 7일 후 순서로 단계별 복습을 진행하세요.',
        '학습 방법: 개념 요약 10분 -> 유사문제 3개 풀이 -> 오답 원인 한 줄 정리.',
        '목표 점수: 다음 평가에서 현재 평균 대비 +10점 향상을 목표로 설정하세요.'
    ].join('\n');
}
...
function renderLearningVisuals() {
    ...
    schedule.forEach((item, idx) => {
        const x = padLeft + (graphW * idx) / (schedule.length - 1);
        const y = padTop + ((100 - item.retention) / 100) * graphH;
        if (idx === 0) ctx.moveTo(x, y);
        else ctx.lineTo(x, y);
    });
    ctx.stroke();
...
500자 이내로 답해주세요.`;
        ...
        let rawAnalysis;
        try {
            rawAnalysis = await callGeminiAPI(analysisPrompt, imageForGemini?.dataUrl || null);
        } catch (analysisError) {
            console.warn('⚠️ AI 분석 API 실패, 로컬 분석으로 대체:', analysisError.message);
            rawAnalysis = '오답 원인: 핵심 개념 적용 과정에서 실수가 발생했습니다.\n개념 설명: 문제의 조건을 먼저 정리한 뒤 공식을 적용하세요.\n해결 방법: 풀이 단계를 1) 조건 정리 2) 계산 3) 검산 순으로 진행하세요.\n유사 문제 팁: 같은 유형 문제 3개를 연속으로 풀며 패턴을 익히세요.';
        }
        ...
        if (document.getElementById('generateProblems')?.checked) {
            try {
                ...
                console.warn('⚠️ 백엔드 문제 생성 실패, 직접 생성으로 폴백:', backendError.message);

                try {
                    const problemPrompt = `
너는 한국어 ${subject} 문제 출제기다.
아래 조건을 만족하는 "관련 유형" 객관식 문제 1개를 생성하라.

[입력]
- 핵심 주제: ${description || problemText || '기본 개념'}
- 학습 수준: ${level}

[출력 규칙]
1) 출력은 JSON 객체 하나만 반환한다. (코드블록/설명문 금지)
2) 키는 반드시 다음 4개만 사용한다:
   - "question": string
   - "options": string[5]
   - "correctIndex": number (0~4)
   - "explanation": string
3) question은 한 문장 이상, options는 서로 다른 선택지 5개, explanation은 2~3문장으로 작성한다.
4) 정답은 options[correctIndex]와 논리적으로 일치해야 한다.
`;

                    const problemJson = await callGeminiAPI(problemPrompt);
                    ...
                    console.warn('⚠️ 직접 생성 폴백도 실패:', fallbackError.message);
                    generatedProblem = createLocalRelatedProblem(description || problemText || subject, subject);
                    problemGenerationWarning = 'AI 유사 문제 생성이 실패하여 관련 유형 기본 문제로 대체했습니다.';
                }
            }
        }
        ...
        if (resultDiv) {
            ...
            resultDiv.innerHTML = `
                <div style="background: #d4edda; border: 1px solid #c3e6cb; padding: 15px; border-radius: 5px; color: #155724;">
                    ...
                    ${problemGenerationWarning ? `<p style="margin-top: 10px; color: #856404; background: #fff3cd; padding: 8px; border-radius: 6px;">⚠️ ${problemGenerationWarning}</p>` : ''}
...
async function generateLearningPlan(options = {}) {
    ...
    try {
        ...
        const prompt = `
사용자의 오답 현황:
- 틀린 문제 수: ${wrongAnswers.length}개
- 푼 문제 수: ${problems.length}개
- 평균 성적: ${scores.length > 0 ? (scores.reduce((a, b) => a + Number(b.percentage || 0), 0) / scores.length).toFixed(1) : '미정'}%

망각 곡선 기반 학습 계획을 만들어주세요:
1. 약점 부분 (3개)
2. 복습 일정 (4개)
3. 학습 방법 (3개)
4. 목표 점수
...
간결하게 답해주세요.`;
        ...
        let plan;
        try {
            plan = sanitizeAiText(await callGeminiAPI(prompt));
        } catch (planError) {
            console.warn('⚠️ 학습 계획 AI 호출 실패, 로컬 계획으로 대체:', planError.message);
            plan = buildLocalLearningPlanText();
        }
